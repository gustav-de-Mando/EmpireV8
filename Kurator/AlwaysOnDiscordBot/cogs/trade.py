...
class Trade(commands.Cog):
    ...
    async def trade_agreement(
        ...
        try:
            ...
            await interaction.response.send_message(
                "Handelsvertrag wurde erstellt. Beide Parteien wurden per DM benachrichtigt.",
                ephemeral=True
            )
    ...
    async def send_trade_confirmation(self, trade_id: str, initiator: discord.Member, partner: discord.Member, embed: discord.Embed):
        try:
            # Send DMs with instructions to respond
            await initiator.send(
                "Bitte antworte mit 'ja' oder 'nein' um den folgenden Handelsvertrag zu bestätigen:",
                embed=embed
            )
            await partner.send(
                "Du hast einen neuen Handelsvertrag erhalten. Bitte antworte mit 'ja' oder 'nein':",
                embed=embed
            )
